...
class LSTMAttention(Config):
    ...
    def add_prediction_op(self):
        ...
        layers = [tf.nn.rnn_cell.DropoutWrapper(tf.nn.rnn_cell.BasicLSTMCell(num_units=self.config.hidden_size),
                     output_keep_prob=self.dropout_placeholder) for _ in range(self.config.n_layers)]
    ...
    def train_on_batch(self, sess, inputs_batch, labels_batch, seqlen_batch):
        ...
        feed = self.create_feed_dict(inputs_batch, labels_batch=labels_batch, seqlen_batch=seqlen_batch, dropout=self.config.dropout)
...
def run_lstm_attention(config, final = False):
    ...
    # For final test combine test and dev.
    if final:
